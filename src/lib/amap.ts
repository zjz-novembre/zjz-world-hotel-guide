...
async function resolveAmapConfig(): Promise<RuntimeAmapConfig> {
  ...
  return {
    ...
    mapStyle: runtimeConfig.mapStyle ?? (import.meta.env.VITE_AMAP_STYLE as string | undefined),
    securityCode:
...
async function readRuntimeAmapConfig(): Promise<RuntimeAmapConfig> {
  try {
    ...
    return {
      ...
      mapStyle: normalizeAmapStyle(config.mapStyle),
      securityCode:
...
function normalizeAmapStyle(value: unknown) {
  if (typeof value !== "string") return undefined;
  const trimmed = value.trim();
  return trimmed.startsWith("amap://styles/") ? trimmed : undefined;
}
